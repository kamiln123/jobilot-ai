...
export default function EditJobOfferPage() {
  ...
  useEffect(() => {
    ...
    async function loadOffer() {
      ...
      const { data, error } = await supabase
        .from("job_offers")
        .select("company_name, position_title, description, requirements, location, work_mode, employment_type, salary_min, salary_max, salary_currency, source_url, notes")
        .eq("id", id)
        .is("deleted_at", null)
        .maybeSingle();
      ...
      setSourceWarning(getSourceWarning(data.source_url ?? ""));
      setState("ready");
    }
    ...
    try {
      const { error } = await getSupabaseBrowserClient()
        ...
        .update({
          ...
          salary_currency: validation.salaryMin === null && validation.salaryMax === null ? null : validation.currency || null,
      ...
      if (error) {
        setSubmitError("Nie udało się zapisać zmian oferty. Spróbuj ponownie.");
        return;
      }

      router.replace(`/job-offers/${id}`);
      router.refresh();
    } catch {
      setSubmitError("Nie udało się zapisać zmian oferty. Spróbuj ponownie.");
    } finally {
  ...
  if (state === "loading") return <main className="grid min-h-screen place-items-center bg-[#f7f7f4] text-sm text-[#687167]">Ładowanie oferty...</main>;
  if (state === "not-found") return <main className="grid min-h-screen place-items-center bg-[#f7f7f4] p-6 text-[#20241f]"><section className="max-w-md rounded-2xl border border-[#e5e7e0] bg-white p-6 text-center"><h1 className="text-xl font-semibold">Nie znaleziono oferty</h1><p className="mt-2 text-sm text-[#687167]">Oferta nie istnieje, została usunięta albo nie masz do niej dostępu.</p><Link className="mt-5 inline-flex text-sm font-semibold text-[#456a4b]" href="/job-offers">Wróć do ofert</Link></section></main>;
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <form className="mt-8 space-y-7 rounded-2xl border border-[#e5e7e0] bg-white p-5 sm:p-8" noValidate onSubmit={handleSubmit}>
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <Field error={errors.companyName} label="Firma *"><input aria-invalid={Boolean(errors.companyName)} autoComplete="organization" className={inputClass} maxLength={160} onChange={(event) => updateValue("companyName", event.target.value)} value={values.companyName} /></Field>
            <Field error={errors.positionTitle} label="Stanowisko *"><input aria-invalid={Boolean(errors.positionTitle)} className={inputClass} maxLength={180} onChange={(event) => updateValue("positionTitle", event.target.value)} value={values.positionTitle} /></Field>
          </div>
          ...
          <Field label="Link źródłowy" note={sourceWarning}><input className={inputClass} inputMode="url" onChange={(event) => updateValue("sourceUrl", event.target.value)} placeholder="https://..." type="text" value={values.sourceUrl} /></Field>
          ...
          <div className="flex flex-col-reverse justify-end gap-3 border-t border-[#edf0e9] pt-6 sm:flex-row">
            <Link className="rounded-xl px-4 py-3 text-center text-sm font-semibold text-[#546052] hover:bg-[#f0f3ed]" href={`/job-offers/${id}`}>Anuluj</Link>
...
function validateOffer(values: FormValues) {
  ...
  if (!values.companyName.trim()) errors.companyName = "Uzupełnij nazwę firmy.";
  if (!values.positionTitle.trim()) errors.positionTitle = "Uzupełnij nazwę stanowiska.";
  if (salaryMin.error) errors.salaryMin = salaryMin.error;
  if (salaryMax.error) errors.salaryMax = salaryMax.error;
  if (!salaryMin.error && !salaryMax.error && salaryMin.value !== null && salaryMax.value !== null && salaryMax.value < salaryMin.value) errors.salaryMax = "Maksymalne wynagrodzenie nie może być niższe niż minimalne.";
  if ((salaryMin.value !== null || salaryMax.value !== null) && !/^[A-Z]{3}$/.test(currency)) errors.salaryCurrency = "Kod waluty powinien mieć trzy litery, np. PLN.";
...
function parseMoney(value: string) {
  ...
  if (!/^\d+(?:[.,]\d{1,2})?$/.test(trimmed)) return { value: null, error: "Podaj liczbę nieujemną z maksymalnie dwoma miejscami po przecinku." };
...
function getSourceWarning(value: string) {
  ...
  if (!sourceUrl) return "";
  if (/\s/.test(sourceUrl)) return "Adres zawiera spacje lub jest niekompletny. Możesz go zapisać jako link, ale nie będzie klikalny.";
  if (!/^https?:\/\//i.test(sourceUrl)) return /^[a-z][a-z\d+.-]*:/i.test(sourceUrl) ? "Ten typ adresu zostanie zapisany jako tekst i nie będzie klikalny." : "Dodaj https://, aby link był później klikalny. Możesz jednak zapisać ofertę bez tej zmiany.";
  try { return new URL(sourceUrl).hostname ? "" : "Adres wygląda na niepełny i nie będzie później klikalny."; } catch { return "Adres wygląda na niepełny i nie będzie później klikalny."; }
}
